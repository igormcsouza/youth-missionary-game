...
class TestCheckPassword:
    ...
    def test_check_password_first_run(self, mock_text_input):
        ...
        mock_text_input.assert_called_once_with(
            "Senha", type="password", on_change=mock_text_input.call_args[1]['on_change'], key="password"
        )
...
class TestHandleDatabaseOperation:
    ...
    @patch('streamlit.info')
    @patch('logging.error')
    def test_handle_database_operation_different_exceptions(self, mock_log_error, mock_st_info):
    ...
    @patch('streamlit.info')
    @patch('logging.error')
    def test_handle_database_operation_long_operation_name(self, mock_log_error, mock_st_info):
